...
- `noise-filename`: Path to file for noise injection. Default: `babble_noise.wav`.
- `decode-snr-target`: Level of signal-to-noise ratio (SNR). Default: `999999` (i.e., clean setting).
- `noise-output`: Optional filepath to save result of noise injection to verify validity. Default: `None`.

- `occlusion-type`: Type of frame-by-frame video corruption to apply. Options: color saturation (`CS`), color contrast (`CS`), block-wise distortion (`BW`), white Gaussian noise in color components (`GNC`), Gaussian blur (`GB`), JPEG compression (`JPEG`). Default: `None`.
- `occlusion-level`: Level of occlusion to apply. Options: `0`, `1`, `2`, `3`, `4`, `5`. Default: `0` (clean setting).
- `occlusion-output`: File path to optionally save first result of occlusion for visualization. If file already exists, it will be overwritten; otherwise, a new file will be created. Default: `None`.

- `include-wer-breakdown`: Flag to include word substitution, insertion, and deletion rates in additional to the overall word edit rate. Default: `False`.
...
--audio-encoder-name openai/whisper-medium.en --unfrozen_modules peft_llm --add_TF_LLM lora \
...
We run the inference for the AVSR pre-trained ckpt in the model zoo and inject `path_to_noise_filename` into the dataset with an SNR of 5. The command is as follows:

```Shell
python eval.py --exp-name AVSR_inference --modality audiovisual --project-wandb wand_project_name \
--pretrained-model-path path_to_avsr_ckpt --root-dir path_to_root_dir --llm-model meta-llama/Meta-Llama-3.1-8B \
--pretrain-avhubert-enc-video-path path_to_avhubert_ckpt --unfrozen_modules peft_llm \
--add_PETF_LLM lora --reduction_lora 64 --alpha 8 --downsample-ratio-video 2 --downsample-ratio-audio 4 \
--max-dec-tokens 32 --num-beams 15 --noise-filename path_to_noise_filename --decode-snr-target 5 
```

**Example 4.**
We run the inference for the AVSR pre-trained ckpt in the model zoo and apply a Gaussian color noise distortion `GNC` to each frame with a level of `4`.
...
--pretrain-avhubert-enc-video-path path_to_avhubert_ckpt --unfrozen_modules peft_llm \
--add_PETF_LLM lora --reduction_lora 64 --alpha 8 --downsample-ratio-video 2 --downsample-ratio-audio 4 \
--max-dec-tokens 32 --num-beams 15 --occlusion-type GNC --occlusion-level 4
```

<details>
  <summary><strong>Audio noise injection and visual distortion details and customization</strong></summary>
- Noise injection utilizes the torchaudio library and is compatible with self-provided or custom noise files. An example of a \
  noise dataset is [**musan**](https://www.openslr.org/17/).
- If the noise file is longer than the files provided in the inference dataset, a random segment of the noise file will be \
  selected and applied to the test data file. This process is repeated for each test data file such that every test file \
  receives a random segment of the noise file, which may create large variations if the noise file is very long. If this does not \
  suit your experimental needs, it is recommended that you modify the `forward` function of the `AddNoise` module \ 
  in `datamodule/transforms.py` to be consistent with your purposes.

- The visual distortions of this repository rely on the [**DeeperForensics-1.0**](https://github.com/EndlessSora/DeeperForensics-1.0.git) \
  library and are limited to the levels and types specified above. To add custom visual distortions, you must manually add them to \
  `datamodule/distortions.py` and `datamodule.add_distortion_to_video.py`.
</details>
...
This repository relies on the [**avhubert**](https://github.com/facebookresearch/av_hubert), [**auto-avsr**](https://github.com/mpc001/auto_avsr), and [**DeeperForensics-1.0**](https://github.com/EndlessSora/DeeperForensics-1.0.git) repositories.
